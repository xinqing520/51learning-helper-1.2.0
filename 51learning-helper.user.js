...
// @license      MIT
// @homepageURL  https://github.com/xinqing520/51learning-helper-1.2.0
// @supportURL   https://github.com/xinqing520/51learning-helper-1.2.0/issues
// @grant        none
// ==/UserScript==
...
(function () {
  ...
  function configFromUi() {
    const config = {
      ...
      deepseekApiKey: $("#cl51-ds-key")?.value,
      deepseekModel: $("#cl51-ds-model")?.value || getConfig().deepseekModel,
      deepseekBaseUrl: ($("#cl51-ds-url")?.value || getConfig().deepseekBaseUrl).replace(/\/+$/, ""),
    };
    if (config.deepseekApiKey === undefined) config.deepseekApiKey = getConfig().deepseekApiKey;
    saveConfig(config);
    return config;
  }
  ...
  function fillAnswers(answers) {
    ...
    for (const q of qs) {
      ...
      if (q.type === "radio") {
        ...
        // 策略1：用选项文本匹配（如选项显示 "A. xxx" 匹配答案 "A"）
        if (letter) {
          radio = q.radios.find((input) => {
            const label = clean(input.parentElement?.textContent || "");
            return label.startsWith(letter + ".") || label.startsWith(letter + ")") || label.startsWith(letter + "、");
          }) || null;
        }

        // 策略2：用 radio 的 value 属性精确匹配
        if (!radio && letter) {
          radio = q.radios.find((input) => String(input.value).toUpperCase() === letter) || null;
        }

        // 策略3：位置匹配 A=第1个 B=第2个 ...
        if (!radio && letter) {
          const idx = letter.charCodeAt(0) - "A".charCodeAt(0);
          if (idx >= 0 && idx < q.radios.length) {
            radio = q.radios[idx];
          }
        }

        // 策略4：直接用答案文本模糊匹配 radio 的 value
        if (!radio) {
          const upperValue = value.toUpperCase();
          radio = q.radios.find((input) => String(input.value).toUpperCase() === upperValue) || null;
        }
  ...
  function callAI(systemPrompt, userMessage, onSuccess, onDone) {
    const config = configFromUi();
    if (!config.deepseekApiKey) {
      setStatus("请先在 AI 排版区域填写 API Key。");
      if (onDone) onDone();
      return;
    }
    const aiStatus = $("#cl51-ai-status");
    if (aiStatus) aiStatus.textContent = "正在请求 AI API...";
    ...
    const url = `${config.deepseekBaseUrl || "https://api.deepseek.com"}/v1/chat/completions`;
    ...
    fetch(url, {
      ...
      .then(({ status, json }) => {
        ...
        if (status !== 200) {
          ...
          if (aiStatus) aiStatus.textContent = `API 错误：${errMsg}`;
          setStatus(`AI API 错误：${errMsg}`);
          return;
        }
        const content = json.choices?.[0]?.message?.content;
        if (!content) {
          if (aiStatus) aiStatus.textContent = "API 返回为空，请重试。";
          setStatus("API 返回内容为空。");
          return;
        }
        onSuccess(content);
      })
      .catch((err) => {
        ...
        if (err.name === "AbortError") {
          if (aiStatus) aiStatus.textContent = "请求超时，请重试。";
          setStatus("AI API 请求超时。");
        } else {
          if (aiStatus) aiStatus.textContent = `网络请求失败：${err.message}`;
          setStatus("API 请求失败，请检查 Base URL 和网络连接。");
        }
      });
  }

  function disableAiButtons() {
    for (const id of ["cl51-ds-format", "cl51-answer-btn", "cl51-answer-fill-btn"]) {
      const btn = $(`#${id}`);
  ...
  function formatAnswersWithAI() {
    const rawAnswers = $("#cl51-answers")?.value?.trim();
    if (!rawAnswers) {
      setStatus("请先把你的答案粘贴到文本框中，再点「AI 排版」。");
      return;
    }
    const qs = questionFields();
    if (!qs.length) {
    ...
    const groups = groupQuestionsByTask(qs);
    const template = groups.map((g) => {
      const lines = [`【${g.key} ${simpleTaskName(g.key)}】`];
    ...
    const systemPrompt = [
      "你是格式整理助手。你的任务是把用户提供的答案，填入指定的答案模板中。",
      "",
      "规则：",
      "1. 仔细阅读题目模板中的每个题型和题号。",
      "2. 从用户提供的答案中，按顺序或按内容匹配到对应的题目。",
      "3. 保持模板的【题型标题】和题号不变，只把答案填在题号后面。",
      "4. 填空题/匹配题/选择题答案通常是单个字母(A/B/C/D)。",
      "5. 翻译题/问答题答案是完整句子，如果用户的答案中某题内容跨了多行，把它们合并到对应题号后。",
      "6. 如果某道题在用户答案中找不到对应内容，在题号后标注（无答案）。",
      "7. 不要输出任何解释，只输出填写好的模板。",
    ].join("\n");
    ...
    callAI(systemPrompt, userMessage, (content) => {
      const textarea = $("#cl51-answers");
      if (textarea) textarea.value = content;
      const aiStatus = $("#cl51-ai-status");
      if (aiStatus) aiStatus.textContent = "AI 排版完成，请检查后点「填入答案」。";
      setStatus("AI 排版完成。请检查答案框内容，确认无误后点「填入答案」。");
    }, enableAiButtons);
  }

  function answerQuestionsWithAI(autoFill) {
    const qs = questionFields();
    if (!qs.length) {
      setStatus("没有找到题目。请先完成阅读，让题目显示出来。");
      return;
    ...
    const questionsDetail = groups.map((g) => {
      let hint = "";
      if (g.key === "vocabulary") hint = "词汇填空：选出正确单词，填选项字母";
      else if (g.key === "matching") hint = "段落匹配：匹配对应段落，答案填段落字母如[A]/[B]";
      else if (g.key === "multiple_choice") hint = "选择题：选择最佳答案，填选项字母";
      else if (g.key === "translation") hint = "翻译题：给出完整译文";
      else hint = "问答题：用完整英文句子回答";

      const lines = [`【${hint}】`];
      for (const q of g.questions) {
        let line = `${q.subIndex + 1}. ${q.prompt}`;
        if (q.options.length) {
          line += "\n    选项：" + q.options.map((o, idx) => `${String.fromCharCode(65 + idx)}. ${o.text}`).join("  ");
        }
        lines.push(line);
      }
      return lines.join("\n");
    ...
    const systemPrompt = [
      "你是一个英语学习助手。请仔细阅读提供的英文文章，然后基于文章内容回答所有题目。",
      "",
      "规则：",
      "1. 所有答案必须基于文章内容，不能脱离文章凭空猜测。",
      "2. 词汇填空题：分析上下文语义选出最合适的选项，答案填选项字母(A/B/C/D)。",
      "3. 段落匹配题：根据题干在文章中找到对应段落，答案填段落字母。",
      "4. 选择题：仔细分析问题和选项，选择最佳答案，填选项字母。",
      "5. 翻译题：根据文章内容准确翻译，中译英给出英文，英译中给出中文。",
      "6. 问答题/写作题：用完整、语法正确的英文句子作答。",
      "7. 保持模板的【题型标题】和题号完全不变，只把答案填在题号后面。",
      "8. 不要输出任何解释、分析或额外文字，只输出填写好答案的模板。",
    ].join("\n");

    const userMessage = [
      "请阅读文章，回答题目。答案必须基于文章内容。",
      "",
      "===== 文章 =====",
      article || "（未能获取文章内容）",
      "",
    ...
    callAI(systemPrompt, userMessage, (content) => {
      ...
      if (textarea) textarea.value = content;
      const aiStatus = $("#cl51-ai-status");
      if (aiStatus) aiStatus.textContent = autoFill ? "AI 答题并已填入，请逐题检查。" : "AI 答题完成，请检查后点「填入答案」。";
      setStatus(autoFill ? "AI 已生成答案并填入。请逐题检查正确性！" : "AI 答题完成。请仔细检查答案，确认无误后点「填入答案」。");
      if (autoFill) {
        const answers = parseAnswers(content);
        if (answers) {
          fillAnswers(answers);
        } else {
          setStatus("AI 生成内容解析失败，请手动点「填入答案」。");
        }
      }
    }, enableAiButtons);
  }
  ...
  function injectUi() {
    ...
    panel.innerHTML = `
      ...
      <div class="section">
        <div class="section-title">AI 排版（支持 OpenAI 兼容接口）</div>
        <div class="grid">
          <label>API Key <input id="cl51-ds-key" type="password" value="${htmlEscape(config.deepseekApiKey || "")}" placeholder="sk-..."></label>
          <label>模型 <input id="cl51-ds-model" value="${htmlEscape(config.deepseekModel || "deepseek-chat")}" placeholder="deepseek-chat" list="cl51-model-list"><datalist id="cl51-model-list"><option value="deepseek-chat"><option value="deepseek-reasoner"><option value="gpt-4o"><option value="gpt-4o-mini"><option value="claude-3.5-sonnet"><option value="qwen-plus"><option value="glm-4"></datalist></label>
        </div>
        <label>Base URL <input id="cl51-ds-url" value="${htmlEscape(config.deepseekBaseUrl || "https://api.deepseek.com")}" placeholder="https://api.deepseek.com"></label>
        <div class="actions compact" style="margin-top:6px;">
          <button id="cl51-answer-btn" type="button" class="primary">AI 答题</button>
          <button id="cl51-answer-fill-btn" type="button">AI答题+填入</button>
          <button id="cl51-ds-format" type="button" class="secondary">AI 排版</button>
        </div>
        <div id="cl51-ai-status" style="font-size:12px;color:#475569;margin-top:4px;"></div>
      </div>
      <div class="section status-box">
        <div id="cl51-timer">计时：未开始</div>
        <div id="cl51-queue">队列：未读取</div>
        <div id="cl51-status">准备好了。AI 答案仅供参考，不会自动提交。</div>
      </div>
    `;
    ...
    $("#cl51-answer-btn").addEventListener("click", () => answerQuestionsWithAI(false));
    $("#cl51-answer-fill-btn").addEventListener("click", () => answerQuestionsWithAI(true));
    $("#cl51-ds-format").addEventListener("click", formatAnswersWithAI);
    for (const id of ["cl51-b", "cl51-u", "cl51-limit", "cl51-display", "cl51-reveal", "cl51-speed", "cl51-ds-key", "cl51-ds-model", "cl51-ds-url"]) {
      const el = $(`#${id}`);
